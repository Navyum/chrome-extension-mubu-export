...
export function syncUiWithState(state) {
  ...
  if (uiState.isExporting) {
    ...
    if (uiState.isPaused) {
      setButtonState(pauseBtn, i18n('resumeExport'), 'btn-continue');
    } else {
      setButtonState(pauseBtn, i18n('pauseExport'), 'btn-pause');
    }
    ...
    if (state.failedFiles && state.failedFiles.length > 0 && retrySection && failedList) {
      ...
      failedList.innerHTML = `<strong>${i18n('failedFilesLabel')}</strong>`;
...
function updateExportTypeIcon() {
  ...
  if (selectIcon) {
    if (iconSrc) {
      ...
      const optionLabel = selectedOption?.textContent?.trim() || i18n('exportFormatLabel');
      selectIcon.alt = optionLabel;
  ...
  if (selectLabel && selectedOption) {
    selectLabel.textContent = selectedOption.textContent?.trim() || i18n('exportFormatLabel');
  }
...
export function showConfetti() {
  const colors = ['#8b5cf6', '#a78bfa', '#10b981', '#f59e0b', '#ef4444', '#3b82f6', '#ec4899', '#fbbf24'];
